...
public class BillingServiceImpl implements BillingService {
    ...
    @Override
    public Exchange creditAccount(TransferInfo info) {
        HttpHeaders headers = new HttpHeaders();
        headers.set(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE);
        ...
        try {
            val result = restTemplate.exchange(url,
                    ...
                    entity,
                    String.class);
            int status = result.getStatusCode().value() == 204 ? 0 : -1;
            return new Exchange(status);
        }
        catch (Exception e) {
            ObjectMapper mapper = new ObjectMapper();
            Message message = null;
            try {
                message = mapper.readValue(e.getMessage(), Message.class);
            } catch (JsonProcessingException ex) {
                throw new RuntimeException(ex);
            }
            return new Exchange(-1, message.getMessage());
        }
    }
}
